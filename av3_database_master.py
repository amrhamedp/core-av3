import numpy as np
import os,re, random,time, MDAnalysis # We use MDAnalysis instead of prody to parse mol2
    ...
    def preprocess_ligands(folder_path):
        for dirpath, dirnames, filenames in os.walk(folder_path):
            for filename in filenames:
                if re.search('.mol2$', filename):
                    ligand_file_path = str(os.path.abspath(dirpath) + "/" + filename)
                    try:
                        MDAnalysis_ligand = MDAnalysis.Universe(ligand_file_path).atoms
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        atom_numbers = map(ligand_atom_to_number, MDAnalysis_ligand.names)
                        coordinates_atoms_charge = np.hstack((MDAnalysis_ligand.positions, np.reshape(atom_numbers, (-1, 1)), MDAnalysis_ligand.charges))
                        np.save(re.sub('.mol2$', '', ligand_file_path), coordinates_atoms_charge)
                    except Exception:
                        pass
        ...
        for dirpath, dirnames, filenames in os.walk(folder_path):
            for filename in filenames:
                if re.search('.mol2$', filename):
                    ...
                    MDAnalysis_receptor = MDAnalysis.Universe(receptor_file_path).atoms

                    def receptor_atom_to_number(atomname):
                        atomic_tag_number = atom_dictionary.ATM[atomname.lower()]
                        return atomic_tag_number
                    try:
                        atom_numbers = map(receptor_atom_to_number, MDAnalysis_receptor.names)
                        coordinates_atoms_charge = np.hstack((MDAnalysis_receptor.positions, np.reshape(atom_numbers, (-1, 1)), MDAnalysis_receptor.charges))
                        np.save(re.sub('.mol2$', '', receptor_file_path), coordinates_atoms_charge)
                    except Exception:
                        pass
...
def docked_ligand_overlaps_with_crystal(database_path,docked_lig_path,tanimoto_cutoff=0.75,clash_cutoff_A=4,clash_size_cutoff=0.3):
    ...
    def calculate_clash(cryst_lig_path, docked_lig_path,clash_cutoff_A=clash_cutoff_A):
        ...
        MDAnalysis_docked_ligand = MDAnalysis.Universe(docked_lig_path).atoms.positions
        MDAnalysis_cryst_ligand = MDAnalysis.Universe(cryst_lig_path).atoms.positions

        def atom_clashing(index):
            # calculate distance to all atoms in the other molecule and check if any of them falls below cutoff
            if any(np.sum((MDAnalysis_cryst_ligand[index,:] - MDAnalysis_docked_ligand)**2,axis=1)**0.5 < clash_cutoff_A):
                return True
            else:
                return False

        clash_mask = map(atom_clashing,np.arange(len(MDAnalysis_cryst_ligand[:, 0])))
